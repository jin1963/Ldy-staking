...
async function claim() {
  ...
  notify("✅ Claimed successfully!", "success");
  refreshDashboard();
}
...
document.getElementById('amountInput').addEventListener('input', async function () {
  ...
  if (!window.web3 || !contract || !accounts || accounts.length === 0 || parseFloat(amount) <= 0) {
    gasEstimateEl.style.display = 'none';
    return;
  }

  try {
    const amountWei = web3.utils.toWei(amount, 'ether');
    const estimatedGas = await contract.methods.stake(amountWei).estimateGas({ from: accounts[0] });
    const gasPrice = await web3.eth.getGasPrice();
    const feeBNB = web3.utils.fromWei((estimatedGas * gasPrice).toString(), 'ether');

    gasEstimateEl.innerText = `Estimated Gas Fee: ~${parseFloat(feeBNB).toFixed(5)} BNB`;
    gasEstimateEl.style.display = 'block';
  } catch (err) {
    gasEstimateEl.style.display = 'none';
  }
});
